...
export async function statusCommand() {
  ...
  try {
    ...
    const pendingDecisions = task.decisions
      ? task.decisions.filter((d: any) => d.status === "PENDING").length
      : 0;
    const resolvedDecisions = task.decisions
      ? task.decisions.filter((d: any) => d.status === "RESOLVED").length
      : 0;
